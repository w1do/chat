...
**Свой мини-Telegram на своём сервере** — для семьи, друзей, команды и
сотрудников. Поднимите собственный мессенджер одной командой и забудьте про
блокировки: он работает, пока работает ваш сервер, и не зависит ни от одного
чужого сервиса.

Открывается в браузере на телефоне и на компьютере — ставить ничего не нужно.
Открытый исходный код, никакой телеметрии и внешнего control plane: приложение
не звонит домой.
...
| | | |
|---|---|---|
| ![Переписка](examples/03-perepiska.png) | ![Список переписок](examples/02-spisok-perepisok.png) | ![Вложения](examples/09-vlozheniya.png) |
| Переписка с реакциями и ответами | Комнаты, диалоги и непрочитанное | Фотографии и документы |

![Рабочий стол](examples/23-rabochiy-stol.png)

Ещё два десятка снимков — приглашения, галерея, поиск, уведомления, настройки,
тёмная тема — в [examples/](examples/README.md).
...
- **Пересказ документа** (по желанию) — ответьте `@ai` на сообщение с
  прикреплённым PDF, DOCX или TXT: помощник прочитает файл и покажет
  пересказ на 500–800 символов. Пересказ видите только вы — в чат он
  попадёт, если нажмёте «Отправить».
...
[w1do.ru](https://w1do.ru) · Telegram: [@W1DO_DIGITAL](https://t.me/W1DO_DIGITAL)
— вопросы по установке, доработкам и внедрению.
