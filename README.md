...
Презентация продукта — [demo.html](demo.html): премиальный code/programming-
лендинг в оранжево-графитово-зелёной палитре, один файл без сборки и запущенного
стека (статус `implemented`). Все 15 разделов имеют локальные сцены; команда
установки печатается с русским декоративным выводом, а ссылки разработчика и
группы настраиваются в `DEMO_CTA_LINKS`. Browser smoke и визуальная приёмка
редизайна отложены до запуска в поддерживаемом Chromium-окружении. Каждая новая
пользовательская функция обновляет сцены тем же изменением.
