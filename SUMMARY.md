...
| chat/rooms-and-messages | Полный срез: комнаты/членство (матрица ролей, единственный owner в БД; владелец правит название и описание и удаляет комнату навсегда одной транзакцией с записью в аудит, архивирование — отдельным адресом `POST /rooms/{room}/archive`, событие `room.deleted.v1` уводит читающих к списку комнат) + сообщения/ответы/реакции/упоминания (санитизация, cursor-пагинация, окно редактирования, soft delete с ответами, идемпотентная отправка, unread-счётчики). 18 package-тестов, 18 feature-тестов, 18 компонентных; OpenAPI 15 путей + клиент; UI: история, композер с reply/mentions, реакции с optimistic rollback; E2E `e2e messaging` | verified |
