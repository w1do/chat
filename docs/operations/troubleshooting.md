...
- **Чат просит войти заново** — токен доступа не истекает сам (ADR-012),
  поэтому повторный вход означает, что токен отозван или не доезжает до
  приложения. Смотрите ответ `GET /api/v1/me`: `401` — токен отозван (выход на
  этом устройстве, сброс пароля, смена пароля с другого устройства). Если
  ответа `401` нет, а вход всё равно теряется, проверьте, что reverse proxy
  передаёт заголовок `Authorization` (некоторые конфигурации его вырезают) и
  что браузер не работает в приватном режиме — там `localStorage` очищается
  вместе с вкладкой.
- **Аватарки и вложения не показываются, а данные приходят** — картинки
  грузятся авторизованным запросом, а не как обычный `<img src>`. Проверьте,
  что `Authorization` доходит до контейнера и что `CORS_ALLOWED_ORIGINS`
  содержит адрес, с которого открыт чат.
